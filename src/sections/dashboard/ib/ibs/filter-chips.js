...
export const _FilterChips = ({
  ...
  const avgDepositChip = useMemo(() => {
    const newChips = filters?.avg_deposit
      ? [
        {
          displayValue: filters?.avg_deposit,
          value: filters?.avg_deposit,
          label: "Min Average Deposit",
        },
      ]
      : [];
    return newChips;
  ...
  const maxAvgDepositChip = useMemo(() => {
    const newChips = filters?.lte_avg_deposit
      ? [
        {
          displayValue: filters?.lte_avg_deposit,
          value: filters?.lte_avg_deposit,
          label: "Max Average Deposit",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_avg_deposit]);
  
  const netFinancialChip = useMemo(() => {
    const newChips = filters?.net_financial
      ? [
        {
          displayValue: filters?.net_financial,
          value: filters?.net_financial,
          label: "Min Net Financial",
        },
      ]
      : [];
    return newChips;
  }, [filters?.net_financial]);

  const maxNetFinancialChip = useMemo(() => {
    const newChips = filters?.lte_net_financial
      ? [
        {
          displayValue: filters?.lte_net_financial,
          value: filters?.lte_avg_deposit,
          label: "Max Net Financial",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_net_financial]);
  
  const totalWithdrawalChip = useMemo(() => {
    const newChips = filters?.total_withdrawal
      ? [
        {
          displayValue: filters?.total_withdrawal,
          value: filters?.total_withdrawal,
          label: "Min Total Withdrawal",
        },
      ]
      : [];
    return newChips;
  }, [filters?.total_withdrawal]);

  const maxTotalWithdrawalChip = useMemo(() => {
    const newChips = filters?.lte_total_withdrawal
      ? [
        {
          displayValue: filters?.lte_total_withdrawal,
          value: filters?.lte_total_withdrawal,
          label: "Max Total Withdrawal",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_total_withdrawal]);

  const lastWithdrawalChip = useMemo(() => {
    const newChips = filters?.last_withdrawal
      ? [
        {
          displayValue: filters?.last_withdrawal,
          value: filters?.last_withdrawal,
          label: "Min Last Withdrawal",
        },
      ]
      : [];
    return newChips;
  }, [filters?.last_withdrawal]);

  const maxLastWithdrawalChip = useMemo(() => {
    const newChips = filters?.lte_last_withdrawal
      ? [
        {
          displayValue: filters?.lte_last_withdrawal,
          value: filters?.lte_last_withdrawal,
          label: "Max Last Withdrawal",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_last_withdrawal]);

  const withdrawalCountChip = useMemo(() => {
    const newChips = filters?.withdrawal_count
      ? [
        {
          displayValue: filters?.withdrawal_count,
          value: filters?.withdrawal_count,
          label: "Min Withdrawal Count",
        },
      ]
      : [];
    return newChips;
  }, [filters?.withdrawal_count]);

  const maxWithdrawalCountChip = useMemo(() => {
    const newChips = filters?.lte_withdrawal_count
      ? [
        {
          displayValue: filters?.lte_withdrawal_count,
          value: filters?.lte_last_withdrawal,
          label: "Max Withdrawal Count",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_withdrawal_count]);

  const totalBetAmountChip = useMemo(() => {
    const newChips = filters?.total_bet_amount
      ? [
        {
          displayValue: filters?.total_bet_amount,
          value: filters?.total_bet_amount,
          label: "Min Total Bet Amount",
        },
      ]
      : [];
    return newChips;
  }, [filters?.total_bet_amount]);

  const maxTotalBetAmountChip = useMemo(() => {
    const newChips = filters?.lte_total_bet_amount
      ? [
        {
          displayValue: filters?.lte_total_bet_amount,
          value: filters?.lte_total_bet_amount,
          label: "Max Total Bet Amount",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_total_bet_amount]);

  const totalWinAmountChip = useMemo(() => {
    const newChips = filters?.total_win_amount
      ? [
        {
          displayValue: filters?.total_win_amount,
          value: filters?.total_win_amount,
          label: "Min Total Win Amount",
        },
      ]
      : [];
    return newChips;
  }, [filters?.total_win_amount]);

  const maxTotalWinAmountChip = useMemo(() => {
    const newChips = filters?.lte_total_win_amount
      ? [
        {
          displayValue: filters?.lte_total_win_amount,
          value: filters?.lte_total_win_amount,
          label: "Max Total Win Amount",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_total_win_amount]);

  const winRateChip = useMemo(() => { 
    const newChips = filters?.win_rate
      ? [
        {
          displayValue: filters?.win_rate,
          value: filters?.win_rate,
          label: "Min Win Rate",
        },
      ]
      : [];
  ...
  const maxWinRateChip = useMemo(() => {
    const newChips = filters?.lte_win_rate
      ? [
        {
          displayValue: filters?.lte_win_rate,
          value: filters?.lte_win_rate,
          label: "Max Win Rate",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_win_rate]);

  const avgBetAmountChip = useMemo(() => { 
    const newChips = filters?.avg_bet_amount
      ? [
        {
          displayValue: filters?.avg_bet_amount,
          value: filters?.avg_bet_amount,
          label: "Min Average Bet Amount",
        },
      ]
      : [];
    return newChips;
  }, [filters?.avg_bet_amount]);

  const maxAvgBetAmountChip = useMemo(() => {
    const newChips = filters?.lte_avg_bet_amount
      ? [
        {
          displayValue: filters?.lte_avg_bet_amount,
          value: filters?.lte_avg_bet_amount,
          label: "Max Average Bet Amount",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_avg_bet_amount]);

  const betCountChip = useMemo(() => { 
    const newChips = filters?.bet_count
      ? [
        {
          displayValue: filters?.bet_count,
          value: filters?.bet_count,
          label: "Min Bet Count",
        },
      ]
      : [];
  ...
  const maxBetCountChip = useMemo(() => {
    const newChips = filters?.lte_bet_count
      ? [
        {
          displayValue: filters?.lte_bet_count,
          value: filters?.lte_bet_count,
          label: "Max Bet Count",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_bet_count]);

  const ggrChip = useMemo(() => { 
    const newChips = filters?.ggr
      ? [
        {
          displayValue: filters?.ggr,
          value: filters?.ggr,
          label: "Min GGR",
        },
      ]
  ...
  const maxGgrChip = useMemo(() => {
    const newChips = filters?.lte_ggr
      ? [
        {
          displayValue: filters?.lte_ggr,
          value: filters?.lte_ggr,
          label: "Max GGR",
        },
      ]
      : [];
  ...
  const lastWithdrawalDateStartChip = useMemo(() => {
    return dateChipVal(filters?.last_withdrawal_date_start, "Last Withdrawal Start Date");
  }, [filters?.last_withdrawal_date_start]);

  const lastWithdrawalDateEndChip = useMemo(() => {
    return dateChipVal(filters?.last_withdrawal_date_end, "Last Withdrawal End Date");
  }, [filters?.last_withdrawal_date_end]);
  ...
  const firstLeadCampaignChip = useMemo(() => {
    const newChips = filters?.first_lead_campaign
      ? [
        {
          displayValue: filters?.first_lead_campaign,
          value: filters?.first_lead_campaign,
          label: "First Lead Campaign",
        },
      ]
      : [];
    return newChips;
  }, [filters?.first_lead_campaign]);

  const lastLeadCampaignChip = useMemo(() => {

    const newChips = filters?.last_lead_campaign
      ? [
        {
          displayValue: filters?.last_lead_campaign,
          value: filters?.last_lead_campaign,
          label: "Last Lead Campaign",
        },
      ]
      : [];
    return newChips;
  }, [filters?.last_lead_campaign]);
  ...
  const chipList = useMemo(() => ([
    ...
    (avgDepositChip?.length > 0 ?
      {
        chip: avgDepositChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ avg_deposit: "" });
        }
      } : null
    ),
    (maxAvgDepositChip?.length > 0 ?
      {
        chip: maxAvgDepositChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_avg_deposit: "" });
        }
      } : null
    ),
    (netFinancialChip?.length > 0 ?
      {
        chip: netFinancialChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ net_financial: "" });
        }
      } : null
    ),
    (maxNetFinancialChip?.length > 0 ?
      {
        chip: maxNetFinancialChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_net_financial: "" });
        }
      } : null
    ),
    (totalWithdrawalChip?.length > 0 ?
      {
        chip: totalWithdrawalChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ total_withdrawal: "" });
        }
      } : null
    ),
    (maxTotalWithdrawalChip?.length > 0 ?
      {
        chip: maxTotalWithdrawalChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_total_withdrawal: "" });
        }
      } : null
    ),
    (lastWithdrawalChip?.length > 0 ?
      {
        chip: lastWithdrawalChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ last_withdrawal: "" });
        }
      } : null
    ),
    (maxLastWithdrawalChip?.length > 0 ?
      {
        chip: maxLastWithdrawalChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_last_withdrawal: "" });
        }
      } : null
    ),
    (lastWithdrawalDateStartChip?.length > 0 ?
      {
        chip: lastWithdrawalDateStartChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ last_withdrawal_date_start: "" });
        }
      } : null
    ),
    (lastWithdrawalDateEndChip?.length > 0 ?
      {
        chip: lastWithdrawalDateEndChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ last_withdrawal_date_end: "" });
        }
      } : null
    ),
    (withdrawalCountChip?.length > 0 ?
      {
        chip: withdrawalCountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ withdrawal_count: "" });
        }
      } : null
    ),
    (maxWithdrawalCountChip?.length > 0 ?
      {
        chip: maxWithdrawalCountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_withdrawal_count: "" });
        }
      } : null
    ),
    (totalBetAmountChip?.length > 0 ?
      {
        chip: totalBetAmountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ total_bet_amount: "" });
        }
      } : null
    ),
    (maxTotalBetAmountChip?.length > 0 ?
      {
        chip: maxTotalBetAmountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_total_bet_amount: "" });
        }
      } : null
    ),
    (totalWinAmountChip?.length > 0 ?
      {
        chip: totalWinAmountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ total_win_amount: "" });
        }
      } : null
    ),
    (maxTotalWinAmountChip?.length > 0 ?
      {
        chip: maxTotalWinAmountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_total_win_amount: "" });
        }
      } : null
    ),
    (winRateChip?.length > 0 ?
      {
        chip: winRateChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ win_rate: "" });
        }
      } : null
    ),
    (maxWinRateChip?.length > 0 ?
      {
        chip: maxWinRateChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_win_rate: "" });
        }
      } : null
    ),
    (avgBetAmountChip?.length > 0 ?
      {
        chip: avgBetAmountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ avg_bet_amount: "" });
        }
      } : null
    ),
    (maxAvgBetAmountChip?.length > 0 ?
      {
        chip: maxAvgBetAmountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_avg_bet_amount: "" });
        }
      } : null
    ),
    (betCountChip?.length > 0 ?
      {
        chip: betCountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ bet_count: "" });
        }
      } : null
    ),
    (maxBetCountChip?.length > 0 ?
      {
        chip: maxBetCountChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_bet_count: "" });
        }
      } : null
    ),
    (ggrChip?.length > 0 ?
    ...
    (firstLeadCampaignChip?.length > 0 ?
      {
        chip: firstLeadCampaignChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ first_lead_campaign: "" });
        }
      } : null
    ),
    (lastLeadCampaignChip?.length > 0 ?
      {
        chip: lastLeadCampaignChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ last_lead_campaign: "" });
        }
      } : null
    ),
    ...
    [filters,
      ...
      maxNetFinancialChip,
      totalWithdrawalChip,
      maxTotalWithdrawalChip,
      lastWithdrawalChip,
      maxLastWithdrawalChip,
      lastWithdrawalDateStartChip,
      lastWithdrawalDateEndChip,
      withdrawalCountChip,
      maxWithdrawalCountChip,
      totalBetAmountChip,
      maxTotalBetAmountChip,
      totalWinAmountChip,
      maxTotalWinAmountChip,
      winRateChip,
      maxWinRateChip,
      avgBetAmountChip,
      maxAvgBetAmountChip,
      ...
      closePnlChip,
      maxClosePnlChip,
      firstLeadCampaignChip,
      lastLeadCampaignChip,
      frdAmountChip,
      maxFrdAmountChip,
      maxTotalCalledChip,
      totalCalledChip,
