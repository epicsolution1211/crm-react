      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Stack
                dangerouslySetInnerHTML={{ __html: thread?.last_message }}
                sx={{
                  "& html": { height: "100%" },
                  "& h1": { fontSize: "14px !important", display: "flex", flexDirection: "column", gap: 2.5 },
                  "& h2": { fontSize: "14px !important" }
                }}
              />
            </Typography>
          </Stack>
          {(thread?.labels?.length > 0 || thread?.unread_count > 0) && (
